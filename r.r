...
# Probability
#     for an event E c O , 0 LE P(E) LE 1
# &    P(o) = 1
...
# probabilities calculated with a marginal total at the numerator and
#    the total group as the denominator result in a marginal probability
#    Given some variable that can be broken down into m categories
#    designated by A1, A2, ..., Ai, ..., Am, and another jointly occurring
#    variable that is broken down into n categories, B1, B2, ..., Bj, ...,
#    Bn, the marginal probability of Ai, mean P(Ai), equals the sum of the
#    joint probabilities of Ai with all the categories of B.
#    P(Ai) = Sigma(P(AinBj)), for all values of j

# complementary events: probability of an event A is equal to 1 minus the
#    probability of its complement
#    P(~A) = 1-P(A)

# exhaustiveness in a probabilistic process allows for all possible events
#    taken together their total probability is 1
#    sum of probabilities of mutually exclusive outcomes equals 1
#    P(E1)+P(E2)+...+P(En)=1

# the probability that a subject picked at random possesses two characteristics
#    at the same time is the intersection or joint probability
#    P(AnB) = joint occurance of A and B

# while EinEj = Ø
# any two mutually exclusive events, the probability of union is their sum
#    note. EinEj = Ø is mutually exclusive and EinEj = 0 is independent
#    P(EiUEj) = P(Ei)+P(Ej) , 
...
# probabilities calculated with a subset of the total group as the denominator
#    result in a conditional probability
#    P(A|B) easier with Bayes

# multiplication rule: a joint probability may be computed as the product of
#    a marginal probability and a conditional probability
...
#        P(B|A)P(A)
# P(A|B) =  ----------
#           P(B)    (alt. den.: P(B|A)P(A)+P(B|~A)P(~A))
...
# a test indicates a negative status when status is positive
# e.g. 1-.8 = 20% of the time a person they have the diease they test negative
#    P(~T|D) = P(~TnD)/P(D)

# False Positive rate = 1-Specificity
# a test indicates a positive status when status is negative
# e.g. 1-.74 = 26% of the time a person tests positive they don't have the disease
#    P(T|~D) = P(Tn~D)/P(~D)

# Predictive Value Positive
#    probability of diseased D given + test
#    P(D|+)=P(D|T)= P(T|D)P(D) / P(T|D)P(D)+P(T|~D)P(~D)

# Predictive Value Negative
#    probability of not diseased ~D given - test
#    P(~D|-)=P(~D|~T)=  P(~T|~D)P(~D) / P(~T|~D)P(~D)+P(~T|D)P(D)
...
#        P(T|D)P(D)    / P(T|D)P(D) + P(T|~D)P(~D)
        Sens*Prev    /( Sens*Prev +  FHits*Infreq )
# P(~D|T) = P(~Tn~D)    /  P(~Tn~D) +  P(DnT)
#        P(~T|~D)P(~D)/( P(~T|~D)P(~D) +  P(~T|D)P(D) )
        Spec*Infreq    /( Spec*Infreq + FNR*Prev )
...
pAnB<-pBgA*pA
#     P(A|B) = P(AnB)/P(B)
Posterior <- pAnB/sum(pAnB); Posterior
...
#Freight carrier operates 100 to 1000 locomotives
#You see locomotive '#321'
#How many locomotives do they have?
...
plot(Posterior,type='l')
round(Posterior,3) #.021 prob of 50, or fair, but this isn't 
#    answering the question we want because it's dependent
#    on the 100-unit granularity of the cutup.
...
#MLE: After 1 'H', the most likely conclusion is
#    that it's a double sided coin.
...
#    p(D|H)/p(D|~H)>1
#    That the prob of the data under the hypothesis
#    is larger than the prob of the data under the not hypothesis
# Coin:
#     Easy: p(D|F) that the coin is fair (e.g. 50%)
#    Hard: p(D|B) that the coin is bias, hard because 'bias' is unspecified
#        which uses the same data twice.
...
#Note how binomial distribution method equivalent
#    to the previous loop method when priors are
#    as they were (i.e. uniform)
pA <- rep(1/101,101)
...
### SAT problem
priors <- read.csv('C:\\WinPython-64bit-3.3.5.0\\scripts\\BayesMadeSimple\\sat_ranks.csv',
            header=F,skip=3)[-(62:64),1:2]
...
raws <- read.csv('C:\\WinPython-64bit-3.3.5.0\\scripts\\BayesMadeSimple\\sat_scale.csv')
#raws[5:17,3:4]
...
#Posteriors: Probability of getting any random question
#        correct (x-axis)
plot(Alice,type='l')
lines(Bob,col=2)
...
library(LearnBayes)
#Step 1. find Beta(a,ß) shape parameters matching knowledge of two quantiles
#    p=value of first and second probability, x=value of first and second qauntile
...
#  value at which the p(random variable) will be less than or equal to q
...
#back-calc SE for two-sided Ex 2: Z-dist
upper <- 1123
lower <- 1077
totalCIspace <- upper-lower
oneCIspace <- totalCIspace/2
...
#back-calc SE for two-sided Ex 3: t-dist
n <- 9
upper <- 1123
lower <- 1077
totalCIspace <- upper-lower
...
#pick 2 balls from the urn without replacement
#get all permutations
...
#Binomial: Two heads out of three flips
...
3/8
#[1] 0.375
choose(3,2)*(.5^2*(1-.5)^(3-2))
#[1] 0.375
dbinom(2,3,.5)
#[1] 0.375
pbirthday(2,classes=3,coincident=2)
